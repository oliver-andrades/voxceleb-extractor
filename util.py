...
class Config(luigi.Config):
    ...
    ram_disk_dir = luigi.Parameter()



def data_out_path(*path):
    path = os.path.join(Config().data_out_dir, '{}{}'.format(Config().dataset, Config().data), *path)
    path = os.path.expanduser(path)
    path = os.path.abspath(path)
    os.makedirs(os.path.dirname(path), exist_ok=True)
    return path

def temp_out_path(*path):
    path = os.path.join(Config().ram_disk_dir, '{}{}'.format(Config().dataset, Config().data), *path)
    path = os.path.expanduser(path)
    path = os.path.abspath(path)
    os.makedirs(os.path.dirname(path), exist_ok=True)
    return path
